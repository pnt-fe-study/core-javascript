...
- 비동기 작업의 동기적 표현
  ...
  - 제어권을 넘겨받은 코드는 다음과 같은 제어권을 가짐
    ...
    2) 콜백 함수를 호출할 때 인자로 넘겨줄 값들 및 그 순서가 정해져있음
    3) 콜백 함수의 this가 무엇을 바라보도록 할지가 정해져 있는 경우도 있음, 정해져 있지 않은 경우 전역객체를 바라보고 임의로 this를 바꾸고 싶을 경우 bind메서드 사용
    4) 어떤 함수에 인자로 메서드를 전달하더라도 이는 결국 함수로서 실행됨
    5) 콜백지옥에서 벗어날 수 있는 방법 - Promise, Generator, async/await 등
